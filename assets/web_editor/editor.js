...
const state = {
  ...
  compareFiles: [],
  compareFile: 0,
  // AI diff summaries, keyed by `base\u0000compare`; `aiDisabled` is set once the
...
// Key for a base/compare pair in `state.aiSummary` (NUL can't appear in a ref).
function aiSummaryKey(base, compare) {
  return `${base}\u0000${compare}`;
}

// Markup for the AI summary block shown above the compare file list. Renders
// from cached state when available, otherwise a loading placeholder; returns
// "" once the feature is known to be disabled so no panel appears at all.
function aiSummaryPanelHtml() {
  if (state.aiDisabled) return "";
  const entry = state.aiSummary[aiSummaryKey(state.compareBase, state.compareTarget)];
  let inner;
  if (entry && entry.html != null) {
    inner = `<div class="ai-summary-body markdown-body">${entry.html}</div>`
      + `<div class="ai-summary-meta">${entry.cached ? "cached" : "generated"}`
      + `${entry.model ? ` · ${escapeHtml(entry.model)}` : ""}</div>`;
  } else if (entry && entry.error) {
    inner = `<div class="ai-summary-error">${escapeHtml(entry.error)}</div>`;
  } else {
    inner = `<div class="ai-summary-loading">Summarising diff…</div>`;
  }
  return `<div class="ai-summary" id="ai-summary"><div class="ai-summary-title">AI summary`
    + `<span class="grow"></span>`
    + `<button type="button" class="ai-chat-btn" id="ai-chat-open">Ask AI</button>`
    + `</div>${inner}</div>`;
}

// (Re)bind the "Ask AI" button — the panel is replaced via outerHTML on refresh,
// so the listener must be reattached each time the markup is rebuilt.
function bindAiChatButton() {
  const btn = document.getElementById("ai-chat-open");
  if (btn) btn.onclick = () => openChat();
}

// Fetch the summary for the current ref pair (once per pair) and patch just the
// panel in place. Server returns `{enabled:false}` (feature off), `{error}` or
// `{summary_html, model, cached}`.
async function loadAiSummary() {
  if (state.aiDisabled) return;
  const base = state.compareBase, compare = state.compareTarget;
  if (!base || !compare) return;
  const key = aiSummaryKey(base, compare);
  if (!state.aiSummary[key]) {
    try {
      const data = await api(
        `/api/ai/summary?base=${encodeURIComponent(base)}&compare=${encodeURIComponent(compare)}`,
      );
      ...
      state.aiSummary[key] = data.error
        ? { error: data.error }
        : { html: data.summary_html || "", model: data.model, cached: data.cached };
    } catch (error) {
      state.aiSummary[key] = { error: error.message };
    }
  }
  // Only patch if the user is still looking at the same comparison.
  const el = document.getElementById("ai-summary");
  if (el && state.compareBase === base && state.compareTarget === compare) {
    el.outerHTML = aiSummaryPanelHtml();
    bindAiChatButton();
  }
}
...
function openChat() {
  ...
  if (!state.compareBase || !state.compareTarget) {
    notify("Pick a base and compare ref first");
    return;
  }
  // Start a fresh conversation whenever the ref pair changed since last opened.
  if (state.chat.base !== state.compareBase || state.chat.compare !== state.compareTarget) {
    state.chat.messages = [];
    state.chat.base = state.compareBase;
    state.chat.compare = state.compareTarget;
  }
...
function renderChatMessages() {
  ...
  if (!state.chat.messages.length) {
    box.innerHTML = `<div class="chat-empty">Ask anything about the `
      + `<code>${escapeHtml(state.chat.base)} … ${escapeHtml(state.chat.compare)}</code> diff.</div>`;
  } else {
    box.innerHTML = state.chat.messages.map(m =>
      m.role === "user"
        ? `<div class="chat-msg chat-user">${escapeHtml(m.content)}</div>`
        : `<div class="chat-msg chat-assistant markdown-body">${m.html || escapeHtml(m.content)}</div>`,
    ).join("");
  }
  if (state.chat.busy) {
    box.innerHTML += `<div class="chat-msg chat-assistant chat-thinking">Thinking…</div>`;
  }
...
async function sendChat() {
  ...
  renderChatMessages();
  const payload = {
    base: state.chat.base,
    compare: state.chat.compare,
    // Send only role/content; the cached `html` is a client-side concern.
  ...
  try {
    ...
    state.chat.messages.push(data.error
      ? { role: "assistant", content: data.error, html: `<div class="chat-error">${escapeHtml(data.error)}</div>` }
      : { role: "assistant", content: data.reply || "", html: data.reply_html || "" });
  } catch (error) {
    state.chat.busy = false;
    state.chat.messages.push({
      role: "assistant", content: error.message,
      html: `<div class="chat-error">${escapeHtml(error.message)}</div>`,
    });
  }
  renderChatMessages();
}
